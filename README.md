...
Mosquito allow us to scan Local Lan or WAN networks using nmap (search-recon) and metasploit (recon-exploration-brute-force), but unlike metasploit the scans performed by nmap will use a false User-Agent (IPhone/Safari) stealth scans and mac address spoofing technics that turns log artifact forensic more dificult to identify the attacker (stealth technics: -sS [syn ack]|--spoof-mac [0]|http.useragent=[..]).

    stealth technics used by mosquito to evade IDS and artifacs logs analizys
    -------------------------------------------------------------------------
